#!/usr/local/bin/bash
# ==========================================
# Automatic Restore Script for CentralIdP PostgreSQL (demo cluster)
...
BACKUP_FILE="$(ls -t "$PROJECT_DIR"/backup/centralidp-postgresql_full_backup_*.sql 2>/dev/null | head -n 1)"
POD="${RELEASE}-centralidp-postgresql-0"
DB_USER=""                                      # DB user
DB_PASSWORD=""                                  # DB password
DATABASE=""
STATEFULSET="${RELEASE}-centralidp-postgresql"  # StatefulSet controlling DB
...
echo "=========================================="
echo "CentralIdP PostgreSQL Automatic Restore"
echo "=========================================="
...
kubectl cp "$BACKUP_FILE" "$NAMESPACE/$POD:/tmp/centralidp_restore.sql"
kubectl exec -n "$NAMESPACE" "$POD" -- ls -lh /tmp/centralidp_restore.sql
...
# --- Automatically truncate tables if data exists (safe version) ---
...
$$;
EOSQL
    echo "Existing data truncated successfully (missing tables ignored)."
else
...
kubectl exec -n "$NAMESPACE" -i "$POD" -- env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d "$DATABASE" -f /tmp/centralidp_restore.sql
echo "Database restored successfully."
echo "------------------------------------------"

# --- Cleanup ---
echo "Removing temporary restore file..."
kubectl exec -n "$NAMESPACE" "$POD" -- rm -f /tmp/centralidp_restore.sql
echo "Cleanup done."
echo "------------------------------------------"
...
echo "Verifying restored data (post-start)..."
kubectl exec -n "$NAMESPACE" -it "$POD" -- \
    sh -c "export PGPASSWORD='$DB_PASSWORD'; psql -U $DB_USER -d $DATABASE -c 'SELECT COUNT(*) FROM public.client;'"
kubectl exec -n "$NAMESPACE" -it "$POD" -- \
    sh -c "export PGPASSWORD='$DB_PASSWORD'; psql -U $DB_USER -d $DATABASE -c 'SELECT COUNT(*) FROM public.identity_provider_config;'"
kubectl exec -n "$NAMESPACE" -it "$POD" -- \
    sh -c "export PGPASSWORD='$DB_PASSWORD'; psql -U $DB_USER -d $DATABASE -c 'SELECT COUNT(*) FROM public.admin_event_entity;'"
kubectl exec -n "$NAMESPACE" -it "$POD" -- \
    sh -c "export PGPASSWORD='$DB_PASSWORD'; psql -U $DB_USER -d $DATABASE -c 'SELECT COUNT(*) FROM public.user_entity;'"
echo "------------------------------------------"
