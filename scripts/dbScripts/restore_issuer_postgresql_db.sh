#!/usr/local/bin/bash
# ==========================================
# Automatic Restore Script for issuer PostgreSQL (demo cluster)
...
BACKUP_FILE="$(ls -t "$PROJECT_DIR"/backup/issuer-postgresql_full_backup_*.sql 2>/dev/null | head -n 1)"
POD="${RELEASE}-issuer-postgresql-0"
DB_USER=""                                     # DB user
DB_PASSWORD=""                                 # DB password
DATABASE=""
STATEFULSET="${RELEASE}-issuer-postgresql"    # StatefulSet controlling DB
...
echo "=========================================="
echo "issuer PostgreSQL Automatic Restore"
echo "=========================================="
...
kubectl cp "$BACKUP_FILE" "$NAMESPACE/$POD:/tmp/issuer-postgresql_restore.sql"
kubectl exec -n "$NAMESPACE" "$POD" -- ls -lh /tmp/issuer-postgresql_restore.sql
...
kubectl exec -n "$NAMESPACE" -i "$POD" -- env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d "$DATABASE" -f /tmp/issuer-postgresql_restore.sql
echo "Database restored successfully."
echo "------------------------------------------"

# --- Cleanup ---
echo "Removing temporary restore file..."
kubectl exec -n "$NAMESPACE" "$POD" -- rm -f /tmp/issuer-postgresql_restore.sql
echo "Cleanup done."
echo "------------------------------------------"
