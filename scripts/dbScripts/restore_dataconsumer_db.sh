#!/usr/local/bin/bash
# ==========================================
# Automatic Restore Script for dataconsumer-1-db PostgreSQL (demo cluster)
...
BACKUP_FILE="$(ls -t "$PROJECT_DIR"/backup/dataconsumer-1-db_full_backup_*.sql 2>/dev/null | head -n 1)"
POD="${RELEASE}-dataconsumer-1-db-0"
DB_USER=""                                      # DB user
DB_PASSWORD=""                                  # DB password
DATABASE=""
STATEFULSET="${RELEASE}-dataconsumer-1-db"    # StatefulSet controlling DB
...
echo "=========================================="
echo "dataconsumer-1-db PostgreSQL Automatic Restore"
echo "=========================================="
...
kubectl cp "$BACKUP_FILE" "$NAMESPACE/$POD:/tmp/dataconsumer-1-db_restore.sql"
kubectl exec -n "$NAMESPACE" "$POD" -- ls -lh /tmp/dataconsumer-1-db_restore.sql
...
kubectl exec -n "$NAMESPACE" -i "$POD" -- env PGPASSWORD="$DB_PASSWORD" psql -U "$DB_USER" -d "$DATABASE" -f /tmp/dataconsumer-1-db_restore.sql
echo "Database restored successfully."
echo "------------------------------------------"

# --- Cleanup ---
echo "Removing temporary restore file..."
kubectl exec -n "$NAMESPACE" "$POD" -- rm -f /tmp/dataconsumer-1-db_restore.sql
echo "Cleanup done."
echo "------------------------------------------"
